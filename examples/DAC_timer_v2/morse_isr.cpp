/*
  ...
  Date:    2020-12-25 created morse_isr.cpp
           2019-12-30 created morse_dac.cpp
  ...
  Purpose: Generate Morse Code through a speaker using onboard 
           ...
           tone from a waveform lookup table.

           All input text is lowercase, all prosigns are uppercase.
  ...
  Allowed audio frequencies:
          If you choose 8 kHz interrupt rate, then it can only generate certain 
          audio frequencies based on number of entries in the waveform lookup table.

          8000 /  4 steps = 2,000 Hz
          8000 /  5 steps = 1,600 Hz
          8000 /  6 steps = 1,333 Hz
          8000 /  7 steps = 1,143 Hz
          8000 /  8 steps = 1,000 Hz
          8000 /  9 steps = 889 Hz
          8000 / 10 steps = 727 Hz
          8000 / 12 steps = 667 Hz
          8000 / 13 steps = 615 Hz
          8000 / 14 steps = 571 Hz
          8000 / 15 steps = 533 Hz
          8000 / 16 steps = 500 Hz
 */
...
#include "morse_isr.h"                // Morse sending class

// ========== interrupt service routine ========================
volatile int wSpeaker = DAC0;         // speaker hardware pin
volatile int wIndex = 0;              // index into waveform table
volatile int wSize = 1;               // sizeof(waveform)
volatile unsigned int* wTable;        // ptr to waveform lookup table, computed once and saved

// singleton function takes no arguments and return void
void dacISR() {

  unsigned int sound = wTable[wIndex];// look up sound level
  analogWrite(wSpeaker, sound);       // output to speaker
...
void DACMorseSenderISR::setup(int toneFrequency) {
  ...
  const int dacOffset = 2048;  // constant adder = exactly half of 2^12
  ...
  for (double phase=0.0; phase<twopi; phase=phase + deltaPhase) {
    float fSample = dacAmplitude * sin(phase) + dacOffset;
    int iSample = (int) fSample; 

    /* *****
    Serial.print(ii);
    Serial.print(". fSample = "); 
    Serial.print(dacAmplitude);
    Serial.print(" * sin(");
    Serial.print(phase);
    Serial.print(") + ");
    Serial.print(dacOffset);
    Serial.print(" = ");
    Serial.print(fSample);
    Serial.print(" = ");
    ...
    ***** */
    ...
    pSamples[ii] = iSample;
    ii++;
    if (ii >= nSamples) break;        // protect from buffer overrun
  }
  
  //Serial.print("end DACMorseSenderISR::setup()" ); Serial.println(__LINE__);
}

void DACMorseSenderISR::start_tone() { // asynchronous ISR, non-blocking
  noInterrupts();
  ::wSpeaker = this->dacPin;          // analog output pin
  ::wIndex = 0;                       // index into waveform table
  ::wSize = this->nSamples;           // length of waveform lookup table
  ::wTable = this->pSamples;          // ptr to waveform lookup table
  interrupts();
  TC.startTimer(this->dacSampleTime, dacISR);
}

void DACMorseSenderISR::stop_tone() {  // asynchronous ISR, non-blocking
  TC.stopTimer();
}

void DACMorseSenderISR::send_dit() {
  float val;
  ...
  for (int count=0; count<cyclesPerDit; count++) {
    ...
    for (int kk=0; kk<nSamples; kk++) {
      val = pSamples[kk];           // lookup DAC setting from the table
...
void DACMorseSenderISR::send_dah() {
  float val;
...
void DACMorseSenderISR::send_dit_space() {
  analogWrite(dacPin, dacOffset);
  delay(iDitDuration);
}
void DACMorseSenderISR::send_letter_space() {
  analogWrite(dacPin, dacOffset);
  int msec = (int)1000.0*letterSpace;
  delay(msec);
}
void DACMorseSenderISR::send_word_space() {
  analogWrite(dacPin, dacOffset);
...
void DACMorseSenderISR::send(char c) {
  if (c == ' ') {
    send_word_space();
  } else {
    char p = getPattern(c);
...
void DACMorseSenderISR::setMessage(const String newMessage) {
	message = newMessage;

}

void DACMorseSenderISR::sendBlocking() {
...
void DACMorseSenderISR::dump() {
  ...
  Serial.println("DAC Morse Sender ISR settings:");
  snprintf(msg, 256, ". DAC nSamples(%d)", nSamples);   Serial.println(msg);
  ...
  ***** */
  for (int ii=0; ii<nSamples; ii++) {
    snprintf(msg, 256, ". pSamples[%d] = %d", ii, pSamples[ii]);   Serial.println(msg);
  }
