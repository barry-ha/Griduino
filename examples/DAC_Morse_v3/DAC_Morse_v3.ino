...
void showFreqSpeed(int frequency, int wpm) {
  ...
  tft.setTextColor(cTEXTCOLOR, cBACKGROUND);
  tft.print("Speed "); 
  tft.setTextColor(cVALUE, cBACKGROUND); tft.print(wpm); tft.print(" wpm ");
  Serial.print(wpm); Serial.println(" wpm");

  tft.setCursor(xLabel, yRow5);
  tft.setTextColor(cTEXTCOLOR, cBACKGROUND); 
  tft.print("Frequency "); 
  tft.setTextColor(cVALUE, cBACKGROUND); tft.print(frequency); tft.print(" Hz ");
...
void setup() {
  ...
  Serial.print("Set wiper position = "); Serial.println( gWiper );
