...
#include <Arduino.h>
//nclude "constants.h"            // Griduino constants and colors
...
int SaveRestore::writeConfig() {
  ...
  result = verifyFolder();    // if needed, create target folder
  if (!result) { return 0; }
...
int SaveRestore::verifyFolder() {
  ...
  if (!gFatfs.exists(sFoldername)) {
    ...
    Serial.print(sFoldername); Serial.println(")");
  }
  return 1;   // indicate success
}
